...
export default function Ventas() {
  ...
  const pedidosDB = data.data?.data?.filter(pedido => pedido.estado === 1)
  ...
  return (
    <div>
        ...
        <p className='text-2xl my-5'>Administra las ventas desde aqui</p>
        ...
        <div className="grid gap-5 2xl:grid-cols-3 lg:grid-cols-2">
            {pedidosDB?.map(pedido => (
                <div key={pedido.id} className="p-5 bg-white shadow space-y-2 border-b"> 
                    ...
                    <p className="text-sm">Folio: <span className="font-semibold">{pedido.id}</span></p>
